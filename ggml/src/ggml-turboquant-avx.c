/**
 * x86_64 AVX2 + FMA3 — hot loops for dot / normalize / scale (no AVX-512).
...
/** <a,b> with FMA-friendly mul-add (no AVX-512). */
static float ggml_turboquant_dot_f32_avx2(const float * a, const float * b, int n) {
...
void ggml_turboquant_vec_normalize_l2_avx(float * x, int n) {
    ...
    ggml_turboquant_vec_scale_inplace_avx(x, n, inv);
}
...
float ggml_turboquant_ip_f32_mse_avx(
    ...
    return ggml_turboquant_dot_f32_avx2(q, xh, d);
}
